...
'''创建随机时间'''
def createTime():
    a1=(2018,10,8,8,0,0,0,0,0)              #设置开始日期时间元组（1976-01-01 00：00：00）
    a2=(2019,1,14,23,59,59,0,0,0)    #设置结束日期时间元组（1990-12-31 23：59：59）
...
'''考虑一个重要的问题，同一时间不能有两个相同的ip访问，因为获取的是外网的ip，而内网对应的外网ip可能就会有重复的了'''
clientIp = ['221.2.164.3','221.2.164.7','221.2.164.9','221.2.164.10',
            '221.2.164.11','221.2.164.12','221.2.164.13','221.2.164.14',
            '221.2.164.15','221.2.164.16','221.2.164.17','221.2.164.18',
            '221.2.164.19','221.2.164.20','221.2.164.21','221.2.164.22',
            '221.2.164.23','221.2.164.24' ,'221.2.164.26','221.2.164.25']

blacklistIP = ['221.2.164.15','221.2.164.16','221.2.164.17','221.2.164.18',]

blackIP = ['221.2.164.12']

'''生成用户登录日志的函数'''   
def createLoginData():
    fp = open("data\\Logindata_.txt",'w')
    #"session":{"realname":"王阳","state":2,"username":"wangyang@wis-eye.com","userlevel":2},
    fp1 = open("data\\userinfo.txt",'r')
    realname = []
    username = []
    password = []
    ...
    s=""
    for i in range(11783):
        s="{"
        k = random.randint(0,75)
        if k<18:
            index = random.randint(0,62)
        elif k<25:
            index = random.randint(0,7)
        elif k<32:
            index = random.randint(7,12)
        elif k<38:
            index = random.randint(13,17)
        elif k<46:
            index = random.randint(19,24)
        elif k<52:
            index = random.randint(26,31)
        elif k<59:
            index = random.randint(34,38)
        elif k<63:
            index = random.randint(46,51)
        elif k<66:
            index = random.randint(57,62)
        elif k<75:
            index = random.randint(37,42)
...
'''检测函数'''
def check():
    fp1 = open("data\\BrowserData_.txt",'r')
...
#     print "plugins个数："+str(len(plugins))
    fp1 = open("data\\BrowserData_.txt",'w')
...
def paixuLogindata_time():
    fp = open("data\\Logindata_.txt",'r')
    fp1 = open("data\\Logindata_paixu.txt",'w')
    lines = {}
    for line in fp.readlines():
        linecopy = line
        line = line.replace("{","").replace("}","").replace("\"","")
...
def paixuBrowserdata_time():
    fp = open("data\\BrowserData_.txt",'r')
    fp1 = open("data\\BrowserData_paixu.txt",'w')
    lines = {}
    for line in fp.readlines():
        linecopy = line
        line = line.replace("{","").replace("}","").replace("\"","")
...
def parseJson():
    ...
    IPS = {}
    locations = {}
    with open("data\\BrowserData_.txt", "r") as f:
    ...
    fp1 = open("data\\userDataTongji.txt",'w')    
    for user in users:
...
def parseJson_combine():
    ...
    fp = open("combineData\\combineData_normal.txt","w")
...
#     parseJson_combine()
    ...
    times.sort(cmp=None, key=None, reverse=False)
    for i in times:
        print i
